...
public class XmlTreeBuilder extends TreeBuilder {
    ...
    protected void initialiseParse(Reader input, String baseUri, Parser parser) {
        ...
        stack.add(doc); // place the document onto the stack. differs from HtmlTreeBuilder (not on
                        // stack)
    ...
    Element insert(Token.StartTag startTag) {
        ...
        // todo: wonder if for xml parsing, should treat all tags as unknown? because
        // it's not html.
        startTag.attributes.deduplicate(settings);

        Element el = new Element.Builder(tag).baseUri(baseUri)
    ...
    void insert(Token.Comment commentToken) {
        ...
        if (commentToken.bogus && comment.isXmlDeclaration()) {
            // xml declarations are emitted as bogus comments (which is right for html, but
            // not xml)
            // so we do a bit of a hack and parse the data as an element to pull the
            // attributes out
            XmlDeclaration decl = comment.asXmlDeclaration(); // else, we couldn't parse it as a decl, so leave as a
                                                              // comment
            if (decl != null)
                insert = decl;
        }
    ...
    void insert(Token.Doctype d) {
        DocumentType doctypeNode = new DocumentType(settings.normalizeTag(d.getName()), d.getPublicIdentifier(),
