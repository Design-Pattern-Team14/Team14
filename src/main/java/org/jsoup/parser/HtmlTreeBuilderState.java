...
enum HtmlTreeBuilderState {
    Initial {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (isWhitespace(t)) {
                ...
                DocumentType doctype = new DocumentType(tb.settings.normalizeTag(d.getName()), d.getPublicIdentifier(),
        ...
        boolean process(Token t, HtmlTreeBuilder tb) {
            ...
            case StartTag:
                ...
                if (name.equals("html")) {
                    ...
                    // else if noscript && scripting flag = true: rawtext (jsoup doesn't run script,
                    // to handle as noscript)
                    tb.insert(start);
    ...
    InHeadNoscript {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isDoctype()) {
                ...
                return anythingElse(t, tb);
            } else if ((t.isStartTag() && StringUtil.in(t.asStartTag().normalName(), "head", "noscript"))
                    || t.isEndTag()) {
                tb.error(this);
                return false;
            } else {
    ...
    InBody {
        boolean process(Token t, HtmlTreeBuilder tb) {
            ...
            case StartTag:
                ...
                if (name.equals("a")) {
                    ...
                    tb.insert(startTag);
                    // todo: If the next token is a U+000A LINE FEED (LF) character token, then
            ...
            case EndTag:
                ...
                if (StringUtil.inSorted(name, Constants.InBodyEndAdoptionFormatters)) {
                    ...
                    for (int i = 0; i < 8; i++) {
                        ...
                        ArrayList<Element> stack = tb.getStack();
                        // the spec doesn't limit to < 64, but in degenerate cases (9000+ stack depth)
                        // this prevents
                        // run-aways
                        ...
                        // todo: Let a bookmark note the position of the formatting element in the list
                        // of active formatting elements relative to the elements on either side of it
                        // in the list.
                        ...
                        for (int j = 0; j < 3; j++) {
                            ...
                            Element replacement = new Element.Builder(
                                    Tag.valueOf(node.nodeName(), ParseSettings.preserveCase)).baseUri(tb.getBaseUri())
                                            .build();
                            // Element replacement = new Element(Tag.valueOf(node.nodeName(),
                            // ParseSettings.preserveCase), tb.getBaseUri());
                            ...
                            if (lastNode == furthestBlock) {
                                // todo: move the aforementioned bookmark to be immediately after the new node
                                // in the list of active formatting elements.
                                // not getting how this bookmark both straddles the element above, but is
                                // inbetween here...
                            }
                        ...
                        Element adopter = new Element.Builder(formatEl.tag()).baseUri(tb.getBaseUri()).build();
                        // Element adopter = new Element(formatEl.tag(), tb.getBaseUri());
                        adopter.attributes().addAll(formatEl.attributes());
                        Node[] childNodes = furthestBlock.childNodes().toArray(new Node[0]);
                        for (Node childNode : childNodes) {
                            adopter.appendChild(childNode); // append will reparent. thus the clone to avoid concurrent
                                                            // mod.
                        }
                        furthestBlock.appendChild(adopter);
                        tb.removeFromActiveFormattingElements(formatEl);
                        // todo: insert the new element into the list of active formatting elements at
                    ...
                    if (!tb.inScope("body")) {
                        ...
                        // todo: error if stack contains something not dd, dt, li, optgroup, option, p,
                ...
                break;
            case EOF:
                // todo: error if stack contains something not dd, dt, li, p, tbody, td, tfoot,
        ...
        boolean anyOtherEndTag(Token t, HtmlTreeBuilder tb) {
            String name = t.asEndTag().normalName; // case insensitive search - goal is to preserve output case, not for
                                                   // the parse to be case sensitive
            ArrayList<Element> stack = tb.getStack();
    ...
    Text {
        ...
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isCharacter()) {
                ...
                // if: An end tag whose tag name is "script" -- scripting nesting level, if
                // evaluating scripts
                tb.pop();
                tb.transition(tb.originalState());
            }
                ...
                if (tb.getPendingTableCharacters().size() > 0) {
                    for (String character : tb.getPendingTableCharacters()) {
                        if (!isWhitespace(character)) {
                            ...
                            tb.error(this);
                            if (StringUtil.in(tb.currentElement().normalName(), "table", "tbody", "tfoot", "thead",
                                    "tr")) {
                                tb.setFosterInserts(true);
    ...
    InCaption {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isEndTag() && t.asEndTag().normalName().equals("caption")) {
            ...
            } else if ((t.isStartTag() && StringUtil.in(t.asStartTag().normalName(), "caption", "col", "colgroup",
                ...
                if (processed)
                    return tb.process(t);
            } else if (t.isEndTag() && StringUtil.in(t.asEndTag().normalName(), "body", "col", "colgroup", "html",
                    "tbody", "td", "tfoot", "th", "thead", "tr")) {
                tb.error(this);
                return false;
            } else {
    ...
    InSelectInTable {
        boolean process(Token t, HtmlTreeBuilder tb) {
            if (t.isStartTag() && StringUtil.in(t.asStartTag().normalName(), "caption", "table", "tbody", "tfoot",
                    "thead", "tr", "td", "th")) {
                tb.error(this);
                tb.processEndTag("select");
                return tb.process(t);
            } else if (t.isEndTag() && StringUtil.in(t.asEndTag().normalName(), "caption", "table", "tbody", "tfoot",
                    "thead", "tr", "td", "th")) {
                tb.error(this);
    ...
    // must make sure these are sorted, as used in findSorted. MUST update
    // HtmlTreebuilderStateTest if more arrays added.
    static final class Constants {
        static final String[] InBodyStartToHead = new String[] { "base", "basefont", "bgsound", "command", "link",
                "meta", "noframes", "script", "style", "title" };
        static final String[] InBodyStartPClosers = new String[] { "address", "article", "aside", "blockquote",
        ...
        static final String[] Formatters = new String[] { "b", "big", "code", "em", "font", "i", "s", "small", "strike",
                "strong", "tt", "u" };
        static final String[] InBodyStartApplets = new String[] { "applet", "marquee", "object" };
        static final String[] InBodyStartEmptyFormatters = new String[] { "area", "br", "embed", "img", "keygen",
                "wbr" };
        ...
        static final String[] InBodyStartDrop = new String[] { "caption", "col", "colgroup", "frame", "head", "tbody",
                "td", "tfoot", "th", "thead", "tr" };
        static final String[] InBodyEndClosers = new String[] { "address", "article", "aside", "blockquote", "button",
                "center", "details", "dir", "div", "dl", "fieldset", "figcaption", "figure", "footer", "header",
                "hgroup", "listing", "menu", "nav", "ol", "pre", "section", "summary", "ul" };
        static final String[] InBodyEndAdoptionFormatters = new String[] { "a", "b", "big", "code", "em", "font", "i",
        ...
        static final String[] InCellCol = new String[] { "caption", "col", "colgroup", "tbody", "td", "tfoot", "th",
                "thead", "tr" };
    }
}
